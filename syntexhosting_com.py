...
HEARTBEAT_INTERVAL = 29       # seconds between each heartbeat
...
# Daily claim (True = enabled, False = disabled)
DAILY_CLAIM_ENABLED = False

# Webhook (set URL here or use --webhook when running)
WEBHOOK_URL     = ""
...
class Bot:
    ...
    def hdrs(self):
        h = {
            ...
            "Origin":            WEB_BASE,
            "Referer":           WEB_BASE + "/billing/rewards/afk",
        }
        if self.token:
    ...
    async def login(self):
        ...
        if s == 200 and isinstance(b, dict):
            ...
            log(f"Login error [{s}]: {str(b)[:200]}", "!!")
        return False

    async def get_tenant(self):
        s, b = await self.req("GET", f"{API_BASE}/api/tenants")
        if s != 200:
            log(f"Failed to get tenant [{s}]: {str(b)[:200]}", "!!")
            return False
    ...
    async def fetch_wallet(self):
        ...
        if s == 200 and isinstance(b, dict):
            ...
            if pe is False and not getattr(self, "_pe_warned", False):
                ...
                log("!! paymentsEnabled=False - can't transfer! Go to Settings to enable it.", "!!")
            return self.wallet
        return self.wallet

    async def fetch_rewards_info(self):
        s, b = await self.req("GET",
            f"{API_BASE}/api/tenants/{self.tenant_id}/rewards")
        if s == 200 and isinstance(b, dict):
            return b
        return {}
    ...
    async def do_daily_claim(self):
        if not self.daily_claim:
            log("Daily claim: DISABLED (DAILY_CLAIM_ENABLED=False)", "..")
            return
        ...
        s, b = await self.req("POST",
            f"{API_BASE}/api/tenants/{self.tenant_id}/rewards/daily/claim",
            json={})
        if s in (200, 201, 204):
            ...
            return
        log(f"Daily claim failed [{s}]: {str(b)[:150]}", "!!")
    ...
    async def afk_start(self):
        s, b = await self.req("POST",
            f"{API_BASE}/api/tenants/{self.tenant_id}/rewards/afk/start",
            json={})
        if s in (200, 201, 204):
    ...
    async def afk_heartbeat(self):
        s, b = await self.req("POST",
            f"{API_BASE}/api/tenants/{self.tenant_id}/rewards/afk/heartbeat",
            json=None)
        return s in (200, 201, 204), s

    async def afk_stop(self):
        s, b = await self.req("POST",
            f"{API_BASE}/api/tenants/{self.tenant_id}/rewards/afk/stop",
            json={})
        log(f"AFK stop [{s}]", "..")
        ...
        try:
            ...
            if c == 's':
                ...
                if ok:
                    ...
                    await self.fetch_wallet()
                    print(f"  New wallet balance: {self.wallet:.2f} coins")
                    await self.webhook(
        ...
        async with aiohttp.ClientSession(
            ...
            if rewards:
                ...
                log(f"AFK rate: {cents_to_coins(rate):.2f} coins/min "
                    f"({afk.get('activeCount', 0)} users currently AFK)", "OK")
                if daily:
            ...
            try:
                while self.running:
                    ...
                    if not ok:
                        log(f"Heartbeat error [{hbs}], restarting in 5s...", "!!")
                        ...
                        while self.running:
                            if await self.afk_start():
                                log("AFK restarted successfully!", "OK")
                                break
                            retry += 1
                ...
                print()
                log("Ctrl+C, shutting down cleanly...", "..")
                ...
                log(f"Heartbeats: {self.hb_count}", "OK")
                log(f"Wallet:     {self.wallet:.2f} coins", "OK")
                print("="*50)


async def main():
    p = argparse.ArgumentParser(description="syntexhosting.com AFK Bot")
    ...
    p.add_argument("--u",          dest="send_to",  default=None,
                   help="Wallet to auto-send coins to when threshold is reached")
    p.add_argument("--c",          dest="send_min", type=float, default=None,
                   help="Auto-send threshold (number of coins)")
    ...
    if a.email == "your@email.com":
        ...
        print("\nExamples:\n"
              ...
              "  python3 afk_bot_syntex.py email pass --u @someone --c 500\n"
              "  python3 afk_bot_syntex.py email pass --webhook https://discord.com/api/webhooks/...")
        return
